...
4. **Visualize regional and macro context.** Likely page/section one of the report. Covers: ecoregion context, watershed context, floodplain context, and an orientation visual whose specific job is confirming to the client that the location being assessed is the correct one — not navigation, not data display, just "yes, this is your parcel." Two prototypes settled 2026-07-28 (regional/state-scale inset, neighborhood-scale main image) — findings, rejected approaches, and open risks in `docs/ILLUSTRATION_NOTES.md`, scripts in `R/illustrate/`. Explicitly prototypes, not final design; this thread is paused, not finished, while focus shifts to step 5.
5. **Visualize parcel-scale data** — what is actually there at the parcel itself, not yet interpreted, and the current focus as of 2026-07-28. DEM raster work (hillshade, slope, TPI) was deliberately reserved for this step after being tried and rejected at the neighborhood scale (see `docs/ILLUSTRATION_NOTES.md`) — it read as "busy" and didn't help a layperson at that wider scale, but may be exactly right zoomed in on the parcel itself. Not started.
